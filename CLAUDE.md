...
要精简准确：不写防御性代码、不提前兼容未知需求、不写回退兜底、兜底构建回退是禁止的，要让 bug 暴露出来，避免过度工程。
...
Queue   (store/queue-logic)  → state.uids snapshot + reinsert + removedUids mask, session-stable
...
2. **Queue is snapshot + reinsert + removedUids mask.** `state.uids` is the single source of truth for ordering — built once per session and never recomputed. Insertion changes go through `reinsert(uid, afterUid, offset)` which directly modifies the snapshot. Display filtering uses only `removedUids` as a mask — cards are never removed from the effective queue just because they are absent from `cardSet` (e.g. completed or scheduled cards). The snapshot and `removedUids` are persisted to `localStorage` (key: `roam-memo:queue:{queueId}`) so page refreshes resume the session; stale entries from other days are auto-cleaned on mount.
...
**Reinsert (Forgot / LBL-Next):** `queueReinsert(uid, afterUid, offset)` directly modifies the `state.uids` snapshot. It removes the uid from its current position and re-inserts it `offset` positions after `afterUid`. This works on the snapshot layer, before the removedUids mask is applied. No separate patch type — reinsertion is a direct state mutation.

**Next mode vs SM2 grading:** Next mode (PROGRESSIVE/FIXED_TIME) and SM2 grading are fully decoupled. `gradeCard` receives an optional `grade` — SM2 passes `grade` (0=Forgot, 1-5=grading), Next mode omits it (`undefined`). The reducer uses `grade !== undefined && grade === 0` to identify Forgot; `grade === undefined` identifies Next mode and enters the LBL Next path. `calculateChildReview`/`calculateNormalReview` pass `sm2_grade` to `generatePracticeData` only when `grade` is provided.
...
- **`computeEffectiveQueue` does NOT filter by cardSet.** Once a card enters the daily queue snapshot, it stays until explicitly removed via `removedUids` or the session ends. Completed/scheduled cards remain visible for undo and review progress tracking. Do NOT add cardSet-based filtering back — it causes LBL decks and completed cards to disappear from the queue after page refresh.
- **Next mode must NOT pass `grade=0`.** Next mode (PROGRESSIVE/FIXED_TIME) passes `grade: undefined` to `gradeCard`. SM2 Forgot passes `grade: 0`. These are distinct — do not conflate them. The reducer distinguishes Forgot (`grade === 0`) from Next (`grade === undefined`) to route to the correct path.
